---
name: harness-design
description: Multi-agent harness for autonomous long-running feature development, modeled on Anthropic's "Harness design for long-running application development" article. Enforces a strict planner → generator → evaluator cycle with file-based handoffs and wraps the generator/evaluator pair in a Ralph loop that iterates until every hard-threshold acceptance criterion passes. USE THIS SKILL whenever the user asks to "build a feature autonomously," "ship this end-to-end without me," "run the harness," "use a planner/generator/evaluator," "use a Ralph loop," or any time the task is a multi-hour feature build that should not stop on the first plausible-looking output. Also use it when a feature spans schema + API + UI + tests + docs and the user wants the agent to keep going until it actually works rather than declaring victory at the first compile.
---
...
The article and the broader literature on agent harnesses converge on a few failure modes worth naming explicitly:
...
- **Compaction instead of fresh context.** For long runs on older models, compaction preserves context-anxiety bias. Fresh context resets between iterations are cleaner. (Less critical on newer models that handle long contexts well, but still worth knowing.)
